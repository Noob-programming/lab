        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            if (txtUserName.Text != string.Empty && txtUserPassword.Text != string.Empty)
            {
                try
                {
                    ...
                    string cons = @"Data Source=" + server_Name + ";Initial Catalog=" + Database_Name + ";Integrated Security=True";
                    SqlConnection con = new SqlConnection(cons);
                    con.Open();
                    string Query = $"select *from Admin where username='{this.txtUserName.Text}' and password ='{this.txtUserPassword.Text}'";
                    ...
                    {
                        MessageBox.Show("خطأ في تسجيل الدخول الرجاء إدخال إسم المستخدم او كلمة المرور بصورة صحيحة", "خطأ في تسجيل الدخول", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        con.Close();
                    }
                }
