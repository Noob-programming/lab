...
namespace lab.User_Control
{
    public partial class UserControlCustomers : UserControl
    {
        ...
        private bool ssmdoule(SqlConnection con)
        {
            try
            {
                con.Open();
                var selector = $@"select *from Guset where [Id_identity] = '{txtMDCardNumber.Text}'";
                var sms = new SqlCommand(selector, con);
                sms.ExecuteNonQuery();
                var db = new SqlDataAdapter(sms);
                var datatable = new DataTable();
                db.Fill(datatable);
                con.Close();
                if (datatable.Rows.Count <= 0) return false;
                MessageBox.Show("have same ssm");
                return true;
            }
            catch (Exception e)
            {
